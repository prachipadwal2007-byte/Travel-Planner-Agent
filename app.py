...
def get_watsonx_model():
    ...
    url        = os.getenv("WATSONX_URL", "https://au-syd.ml.cloud.ibm.com").strip().rstrip("/")

    # Guard: project_id must be a clean UUID (hex + hyphens only)
    if project_id and not re.fullmatch(r"[0-9a-f\-]{36}", project_id):
        raise ValueError(
            f"WATSONX_PROJECT_ID '{project_id}' contains invalid characters. "
            "It must be a plain UUID like: f95806b9-3203-4fea-8fb5-72e36c3037e2"
        )
